...
class Appointment extends Lib\Base\Entity
{
    ...
    /**
     * Synchronize Google Calendar with appointment.
     */
    public function syncGoogleCalendar()
    {
        $google = new Lib\Google\Client();
        if ( $google->authWithStaffId( $this->getStaffId() ) ) {
            $google->calendar()->syncAppointment( $this );
        }
    }
    ...
    public function deleteGoogleCalendarEvent()
    {
        ...
        if ( $google->authWithStaffId( $this->getStaffId() ) ) {
            ...
            return $google->calendar()->deleteEvent( $this->getGoogleEventId() );
        }
