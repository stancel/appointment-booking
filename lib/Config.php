 ...
 *
 *
 * @method static bool advancedGoogleCalendarEnabled() Check whether Advanced Google Calendar add-on is enabled or not.
 * @method static bool authorizeNetEnabled()           Check whether Authorize.Net add-on is enabled or not.
 * @method static bool cartEnabled()                   Check whether Cart add-on is enabled or not.
 ...
 * @method static bool customerInformationEnabled()    Check whether Customer Information add-on is enabled or not.
 ...
 * @method static bool invoicesEnabled()               Check whether Invoices add-on is enabled or not.
 ...
 * @method static bool ratingsEnabled()                Check whether Ratings add-on is enabled or not.
...
abstract class Config
{
    ...
    public static function getCaSeSt()
    {
        ...
        if ( ! Proxy\Locations::getAllowServicesPerLocation() ) {
    ...
    /**
     * Whether to show birthday field.
     *
     * @return bool
     */
    public static function showBirthday()
    {
        return (bool) get_option( 'bookly_app_show_birthday', false );
    }
    ...
    {
        return (bool) get_option( 'bookly_app_show_address', false );
    }
    ...
    {
        return (bool) get_option( 'bookly_app_show_facebook_login_button', false );
    }
    ...
    /**
     * Get Google Calendar synchronization mode (false means Google Calendar integration is not configured).
     *
     * @return string|false  1-way, 1.5-way, 2-way
     */
    ...
    /**
     * Whether to show price for payment methods.
     *
     * @return bool
     */
    public static function showPaymentSpecificPrices()
    {
        ...
        if ( ! $show_corrected_price ) {
            $show_corrected_price = Proxy\Shared::showPaymentSpecificPrices( $show_corrected_price );
        }
    ...
    {
        return self::cartEnabled() && ! Config::wooCommerceEnabled();
    }
        ...
        if ( $result === null ) {
            ...
            if ( $server_error_period > 7 ) {
                ...
                foreach ( $bookly_plugins as $plugin_slug => $plugin_class ) {
                    if ( $plugin_class::getPurchaseCode() == '' && $plugin_class::enabled() && ! $plugin_class::embedded() ) {
                        $grace_start = (int) get_option( $plugin_class::getPrefix() . 'grace_start' );
                        $expiration_day = (int) ( $grace_start / DAY_IN_SECONDS ) + $grace_period_days;
                        if ( $unix_day >= $expiration_day ) {
                            $result['bookly'] = 'expired';
