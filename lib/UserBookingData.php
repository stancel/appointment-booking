...
class UserBookingData
{
    ...
    public function __construct( $form_id )
    {
        ...
        if ( $current_user && $current_user->ID ) {
            ...
            $this
                ...
                ->setBirthday( $_COOKIE['bookly-cst-birthday'] )
    ...
    public function validate( $data )
    {
        ...
        foreach ( $data as $field_name => $field_value ) {
            switch ( $field_name ) {
                ...
                case 'birthday_day':
                    $validator->validateBirthdayDay( $field_name, $data );
                    break;
                case 'birthday_month':
                    $validator->validateBirthdayMonth( $field_name, $field_value );
                    break;
                case 'birthday_year':
                    $validator->validateBirthdayYear( $field_name, $field_value );
                    break;
    ...
    public function getCustomer()
    {
        if ( $this->customer === null ) {
            ...
            if ( ! $this->customer->isLoaded() ) {
                if ( Config::showFacebookLoginButton() && $this->getFacebookId() ) {
    ...
    {
        switch ( $field_name ) {
            case 'additional_address': return $this->additional_address;
    ...
    public function setBirthday( $birthday )
    {
        if ( is_array( $birthday ) ) {
            $day        = isset( $birthday['birthday_day'] ) ? (int) $birthday['birthday_day'] : 1;
            $month      = isset( $birthday['birthday_month'] ) ? (int) $birthday['birthday_month'] : 1;
            $year       = isset( $birthday['birthday_year'] ) ? (int) $birthday['birthday_year'] : date( 'Y' );
