...
class Finder
{
    ...
    private function _prepareStaffData()
    {
        ...
        $query = Lib\Proxy\Locations::prepareStaffServiceQuery( $query );
        if ( ! Lib\Proxy\Locations::getAllowServicesPerLocation() ) {
            $query
        ...
        if ( Lib\Config::getGoogleCalendarSyncMode() == '1.5-way' ) {
            ...
            foreach ( $query->find() as $staff ) {
                ...
                if ( $google->auth( $staff ) ) {
                    foreach ( $google->calendar()->getBookings( $this->start_dp ) ?: array() as $booking ) {
