...
class Controller extends Lib\Base\Controller
{
    ...
    public function index()
    {
            ...
            if ( $this->csrfTokenValid() ) {
                switch ( $this->getParameter( 'tab' ) ) {
                    ...
                    case 'google_calendar':  // Google calendar form.
                        $alert = Lib\Proxy\AdvancedGoogleCalendar::preSaveSettings( $alert, $this->getPostParameters() );
                        ...
                        update_option( 'bookly_gc_sync_mode', $this->getParameter( 'bookly_gc_sync_mode' ) );
                    ...
                    case 'facebook':  // Facebook.
                        update_option( 'bookly_fb_app_id', $this->getParameter( 'bookly_fb_app_id' ) );
                        $alert['success'][] = __( 'Settings saved.', 'bookly' );
                        break;
                    case 'customers':  // Customers form.
                        ...
                        update_option( 'bookly_cst_required_address',           $this->getParameter( 'bookly_cst_required_address' ) );
                        update_option( 'bookly_cst_address_show_fields',        $this->getParameter( 'bookly_cst_address_show_fields', array() ) );
                        ...
                        if ( $this->getParameter( 'bookly_cst_create_account' ) && ! in_array( 'email', $bookly_cst_required_details ) ) {
        ...
        $payments     = array();
        $payment_data = array(
            'local'  => $this->render( '_payment_local', array(), false ),
            'paypal' => $this->render( '_payment_paypal', array(), false ),
        );
        $payment_data = Lib\Proxy\Shared::preparePaymentGatewaySettings( $payment_data );
        ...
        $this->render( 'index', compact( 'candidates', 'values', 'grace_remaining_days', 'payments' ) );
    }
