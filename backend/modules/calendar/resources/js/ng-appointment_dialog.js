;(function() {
    ...
    module.factory('dataSource', function($q) {
        var ds = {
            ...
            findTime : function(source, value) {
                var result = null,
                    time = source == 'start' ? ds.getDataForStartTime() : ds.getDataForEndTime();
            ...
            getDataForEndTime : function() {
                ...
                if (ds.form.start_time) {
                    ...
                    if (
                        ...
                        result.every(function (item) {return item.value !== ds.data.app_end_time.value;})
                    ) {
    ...
    module.controller('appointmentDialogCtrl', function($scope, $element, dataSource) {
        ...
        $scope.attachPayment = function (customer) {
            var $dialog = jQuery('#bookly-payment-attach-modal');
            jQuery('#bookly-attach-payment-apply', $dialog).data('payment-customer_id', customer.id);
            jQuery('.bookly-js-attach-payment-id', $dialog).val('');
            $dialog.modal({show: true})
                .on('hidden.bs.modal', function () {
                    jQuery('body').addClass('modal-open');
                });
